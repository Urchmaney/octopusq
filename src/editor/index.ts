...
export class DocumentEditor {
	...
	constructor(initialContent: PartialBlock[] , private documentId: string) {
		...
		let debouncedUpdate = debounce(this.updateDocument, 5000);
		this._blocknoteEditor.onChange((editor) => (debouncedUpdate(editor.document)));
	...
	async updateDocument(document: any) {
		...
		console.log(document, this.documentId, "sent");
	}
